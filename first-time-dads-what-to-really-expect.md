...
- If an epidural bolus (refresh of the epidural) is requested, it's possible to request a small one. Also possible to start with half of one. This makes it easier to feel and push. Dad blocks access to the epidural IV in case a cranky anesthesiologist tries to give a full bolus without permission, which would delay pushing.
...
- After pumping, dad can clean equipment while mom applies lanolin cream and silver shields (ensure no moisture underneath either!). Either holds baby upright and burps for 15 minutes.
- Change diaper if you heard anything - apply zinc diaper cream after drying if there are red spots.
